...
export class Diagnostics {
  ...
  static logConnectionFailure(serverUrl: string, error?: any) {
    ...
    console.log('🔍 가능한 원인:');
    console.log('');
    console.log('1️⃣  백엔드 서버 미실행');
    console.log('   → 터미널에서 확인: lsof -i :3000');
    console.log('   → 실행: cd allsub-backend && npm run start:dev');
    console.log('');
    console.log('2️⃣  잘못된 URL 설정');
    console.log('   → 현재 URL:', serverUrl);
    if (Platform.OS === 'ios') {
      console.log('   → iOS는 localhost:3000 사용해야 함');
      if (serverUrl.includes('10.0.2.2')) {
        console.log('   ⚠️  10.0.2.2는 Android용입니다!');
      }
    } else if (Platform.OS === 'android') {
      console.log('   → Android는 10.0.2.2:3000 사용해야 함');
      if (serverUrl.includes('localhost')) {
        console.log('   ⚠️  localhost는 iOS용입니다!');
      }
    }
    console.log('');
    console.log('3️⃣  방화벽 차단');
    console.log('   → macOS: 시스템 환경설정 → 보안 → 방화벽');
    console.log('   → Node.js 허용 확인');
    console.log('');
    console.log('4️⃣  네트워크 문제');
    console.log('   → VPN 사용 중이면 비활성화');
    console.log('   → WiFi 연결 확인');
    console.log('');
    console.log('5️⃣  포트 충돌');
  ...
  static logSuccessInfo() {
    ...
    console.log('');
    console.log('🏝️  Live Activities (iOS 16.1+):');
